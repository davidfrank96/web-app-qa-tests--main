...
begin
  ...
  perform pg_temp.reject('select public.retention_enable_monthly(''evidence-retention-v2'')','v3 policy');
  ...
  perform public.retention_enable_monthly('evidence-retention-v3');
  ...
  reset role;
  raise notice 'PASS: v3 RLS, shared QA mutex, at-most-once occurrence, hold race, safety gate, ownership, verified absence before prune, partial retry, all budgets, tombstone, expiry publication block, exact accounting, run/audit/cleanup/monitoring preservation';
end $$;

-- Monthly scheduling is deterministic and independently enforced by the database.
do $$ declare t text; count_before bigint; next_time timestamptz; begin
 if to_regprocedure('public.retention_enable_daily()') is not null or to_regprocedure('public.retention_claim_occurrence(text,text,boolean)') is not null then raise exception 'Legacy daily entry point exists'; end if;
 perform public.retention_disable_automatic();
 if (public.retention_health()->>'enabled')::boolean then raise exception 'Automatic not disabled'; end if;
 select count(*) into count_before from public.retention_occurrences;
 perform public.retention_enable_monthly('evidence-retention-v3');
 if (select count(*) from public.retention_occurrences)<>count_before then raise exception 'Enable triggered cleanup'; end if;
 next_time:=(public.retention_health()->>'nextScheduledAt')::timestamptz;
 if next_time<=now() or extract(day from next_time at time zone 'Europe/Dublin')<>1 or (next_time at time zone 'Europe/Dublin')::time<>time '01:30' then raise exception 'Next occurrence incorrect'; end if;
 if public.retention_monthly_occurrence('2026-10-01 00:30:59Z','2026-09-01Z','2026-09-16Z') is distinct from 'monthly:2026-10' then raise exception 'Summer DST scheduling failed'; end if;
 if public.retention_monthly_occurrence('2026-12-01 01:30Z','2026-09-01Z','2026-09-16Z') is distinct from 'monthly:2026-12' then raise exception 'Winter scheduling failed'; end if;
 foreach t in array array['2026-10-01 00:29:59Z','2026-10-01 00:31Z','2026-10-01 12:00Z','2026-10-02 00:30Z','2026-10-25 01:30Z'] loop
  if public.retention_monthly_occurrence(t::timestamptz,'2026-09-01Z','2026-09-16Z') is not null then raise exception 'Catch-up allowed: %',t; end if;
 end loop;
 if public.retention_monthly_occurrence('2026-10-01 00:30:45Z','2026-10-01 00:30:01Z','2026-09-16Z') is not null or
    public.retention_monthly_occurrence('2026-10-01 00:30:45Z','2026-09-01Z','2026-10-01 00:30:01Z') is not null then raise exception 'Startup or configuration catch-up allowed'; end if;
 if public.retention_claim_occurrence('monthly:2026-10','fixture-owner',true,null)->>'status'<>'NOT_DUE' then raise exception 'Missing scheduler start accepted'; end if;
 perform pg_temp.reject('select public.retention_claim_occurrence(''daily:2026-09-16'',''fixture-owner'',true)','Invalid retention occurrence');
 raise notice 'v3 monthly no-catch-up, policy and execution safety regressions PASS';
end $$;
