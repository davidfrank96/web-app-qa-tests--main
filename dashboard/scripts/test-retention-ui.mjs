...
const bundle = await build({ stdin: { contents: `import React from 'react'; import {createRoot} from 'react-dom/client'; import {InssaOpsClient} from './inssa-ops-client';
  createRoot(document.getElementById('root')).render(React.createElement(InssaOpsClient, {currentUser:{id:'fixture',email:'fixture@example.test',role:new URLSearchParams(location.search).get('role')},initialCampaignDefinitions:[],initialMetadataBackend:${JSON.stringify(backend)},initialRuns:[]}));`,
...
const server = createServer((req, res) => {
  ...
  if (req.url === '/api/retention') { assert.equal(req.method, 'GET'); retentionReads++; res.statusCode = fail ? 503 : 200;
    body = fail ? { error: 'Fixture failure' } : { mode: 'DRY RUN ONLY', policyVersion: 'evidence-retention-v1', asOf: '2026-09-14T22:00:00Z', reviewReasons: [], summary, bundles: [] }; }
  else if (req.url === '/api/runs') body = { runs: [], metadataBackend: backend };
...
try {
  ...
  assert.equal(retentionReads,3); assert.deepEqual(errors,[]);
  console.log('PASS: real Operations admin visibility, explicit GET only, no polling, summary, single Dry Run/Refresh action, stale-plan clearing and retry');
