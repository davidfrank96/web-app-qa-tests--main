...
async function main() {
  ...
  if (command === "--compare" && !occurrence) {
    ...
    certifyExecutionPlan(comparison.twentyOneDays);
    process.stdout.write(`${JSON.stringify(comparison, null, 2)}\n`); return;
  }
  if (command === "--enable-daily" && !occurrence) {
    await retentionRpcClient()("retention_enable_daily", {}); process.stdout.write('Daily retention enabled: 01:30 Europe/Dublin.\n'); return;
  }
  if (command !== "--execute" || !/^manual:[a-zA-Z0-9_-]+$/.test(occurrence ?? "")) throw new Error("Usage: retention:execute --compare | --execute manual:OCCURRENCE | --enable-daily");
